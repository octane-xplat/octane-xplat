...
> `Switch`/`Slider`/`ActivityIndicator`/`Tabs`/`Drawer` are self-drawn;
...
> `LiquidGlassContainer`) and `ui/web` (`Hoverable`). Shared `List`,
> `Modal`/`openModal`, `KeyboardAvoiding`-on-web, the `glass` prop, and
...
| `ActivityIndicator`         | `vx-spinner` border ring + keyframes                      | `vx-spinner` border ring + `el.animate()` rotate loop                                          | `busy=false` hides on both                                                                                                                                                                                                                                                                                                                                                                                              |
| `Modal` [platform]          | — *(removed from shared)*                                 | **`showModal` + a second Octane root** — `UIModal`/`MaterialDialog` in subpaths                | ⚠ context does NOT cross roots — see below. Shared temporary surfaces: `Sheet`, `Overlay`, `openSheet`                                                                                                                                                                                                                                              |
| `SafeArea`                  | CSS `env(safe-area-inset-*)` padding                      | root-level padding + `iosOverflowSafeArea` management                                          | plus `useSafeAreaInsets()` hook                                                                                                                                                                                                                                                                                                                                                                                         |
| `KeyboardAvoiding` [native-only] | — *(web leaf removed — was inert)*                   | scrollview + `input-accessory`/inset management                                                | iOS vs Android differ internally — acceptable leaf complexity                                                                                                                                                                                                                                                                                                                                                           |
...
| `Hoverable` [web-only]      | delayed hover intent + body-portal Popover (`ui/web`)       | — *(native leaf removed)*                                           | Touch has no hover; the long-press stand-in was fake parity. Web keeps the card alive while the pointer crosses from the anchor to the portaled card. |
...
| `LiquidGlass` [ios-only]    | — *(removed from shared)*                                 | `liquidglass` (registered) — root IS an interactive `UIVisualEffectView`+`UIGlassEffect` (`ui/ios`) | real material on iOS 26+ only; inert layout on iOS <26, nothing on Android. The web backdrop-filter approximation was fake parity and is deleted. See the Liquid Glass section                                                                                                                                                                                                                                                                                                                      |
| `LiquidGlassContainer` [ios-only] | — *(removed from shared)*                         | `liquidglasscontainer` (registered) — `UIGlassContainerEffect` on AbsoluteLayout (`ui/ios`)      | merged-glass region — glass siblings morph together across `spacing` dips; children position via `left`/`top`                                                                                                                                                                                                                                                         |
