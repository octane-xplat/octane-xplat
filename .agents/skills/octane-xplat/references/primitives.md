...
| `Tabs`          | `className, style, id, tabs: TabSpec[], selectedIndex?, onSelectedIndexChanged?, resolveScreen?`                    | Button-row + pane (web) / `TabView` (native). See navigation.md for `stack` panes + outlets. `TabSpec.icon` names a registered glyph: web/Android render it via `Icon`; iOS needs the glyph's `src` (`sys://` SF Symbol, `res://`, `font://`, file) or `font`/`text` for `UITabBarItem`.                                                                     |
